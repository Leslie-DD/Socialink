...
import com.hnu.heshequ.adapter.listview.WwSecondAdapter;
...
public class WwSecondActivity extends NetWorkActivity implements View.OnClickListener, XRecyclerView.LoadingListener {
    ...
    private void init() {
        ...
        adapter.setListener(new WwSecondAdapter.DelListener() {
            @Override
...
//                imm.showSoftInput(etContent,InputMethodManager.SHOW_FORCED);
            ...
            case R.id.tvDing:
                setBodyParams(new String[]{"id"}
                        , new String[]{bean.id + ""});
                sendPost(WenConstans.WwDing, 103, WenConstans.token);
                break;
        }
    }
    ...
    private void getDisscuss(int where) {
        ...
        sendPost(WenConstans.WwSecond, where, WenConstans.token);
    }
